...
@interface RewardVideoViewController () <LingAggRewardVideoAdDelegate>
...
@property (nonatomic, strong) AdbidRewardVideoTokenTester *tokenTester;
...
- (void)viewDidLoad {
    ...
    self.tokenTester = [[AdbidRewardVideoTokenTester alloc] init];
...
- (void)loadC2SButtonTapped {
    ...
    NSString *slotId = self.slotIdTextField.text;
    if (slotId.length > 0) {
        [[NSUserDefaults standardUserDefaults] setObject:slotId forKey:@"DemoRewardVideoAdID"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }

    [self setupRewardVideoAd];
    [self addLog:[NSString stringWithFormat:@"开始加载激励视频C2S广告，广告位ID: %@", slotId ?: @""]];
...
//
//    [LingAdSDKManager requestServerBidTokenConfigForPositionId:slotId completion:^(NSString * _Nullable sdkInfoConfig, NSError * _Nullable configError) {
...
//        NSString *configLog = [NSString stringWithFormat:@"requestServerBidTokenConfigForPositionId 耗时: %.2f ms, slotId=%@, %@",
//                               elapsedTime,
//                               slotId ?: @"",
//                               configError ? [NSString stringWithFormat:@"error=%@", configError.localizedDescription ?: @"unknown"] : @"success"];
//        [strongSelf addLog:configLog];
//
...
//            NSString *tokenLog = [NSString stringWithFormat:@"requestServerBidTokenConfigBeforeLoadForRewardVideoAd 总耗时: %.2f ms, slotId=%@, %@",
//                                  methodElapsedTime,
//                                  slotId ?: @"",
//                                  error ? [NSString stringWithFormat:@"error=%@", error.localizedDescription ?: @"unknown"] : @"success"];
//            [strongSelf addLog:tokenLog];
//
//            if (!success || error) {
...
//            }
//
//            if (strongSelf.rewardVideoAd != rewardVideoAd) {
//                [strongSelf addLog:@"忽略已失效的C2S Token请求结果"];
//                return;
//            }
//
...
//            });
//            [rewardVideoAd loadAdWithToken:token];
//        }];
//    }];
}
