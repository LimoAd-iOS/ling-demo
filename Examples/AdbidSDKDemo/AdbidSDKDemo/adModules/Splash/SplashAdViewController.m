...
/// 临时调试：开屏写死广告配置。看完页面改回 0
#ifndef LM_SPLASH_DEBUG_HARDCODE_AD
#define LM_SPLASH_DEBUG_HARDCODE_AD 1
#endif


@interface SplashAdViewController () <LingAggSplashAdDelegate>
...
- (void)loadButtonTapped:(UIButton *)sender {
    ...
    [self addLog:message];
    self.splashAd = [[LingAggSplashAd alloc] initWithSlotId:self.adIdTextField.text];
...
- (void)runServerBidTokenConfigCostTestWithSlotId:(NSString *)slotId {
    ...
    LingAggSplashAd *testSplashAd = [[LingAggSplashAd alloc] initWithSlotId:slotId];
...
- (void)showButtonTapped:(UIButton *)sender {
    if (self.splashAd && [self.splashAd isReady]) {
        NSString* message = @" 正在展示广告（带底部品牌条）";
        self.statusLabel.text =  [self statusLog:message];
        [self addLog:message];
...
- (UIView *)demoSplashBrandView {
    ...
    UIView *logoBox = [[UIView alloc] initWithFrame:CGRectZero];
