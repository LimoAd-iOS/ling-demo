...
@interface BottomNativeAdViewController () <LingAggNativeAdDelegate,LingAggNativeMediaViewDelegate,LingAggRewardVideoAdDelegate,LingAggSplashAdDelegate>
...
- (void)setupConstraints {
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.controlPanel.heightAnchor constraintEqualToConstant:490]
    ]];
...
- (void)loadC2SButtonTapped:(UIButton *)sender {
    ...
    NSLog(@"开始加载底通信息流C2S广告，广告位ID: %@", slotId);
    ...
    [LingAggSDKManager requestServerBidTokenConfigForPositionId:slotId completion:^(NSString * _Nullable sdkInfoConfig, NSError * _Nullable configError) {
        ...
        [strongSelf.tokenTester getTokenWithAdId:slotId sdkInfo:sdkInfoConfig completion:^(BOOL success, NSDictionary * _Nullable config, NSError * _Nullable error) {
            if (strongSelf.nativeAd != nativeAd) {
                NSLog(@"忽略已失效的底通信息流C2S Token请求结果");
                return;
            }
...
- (void)clearCurrentNativeAdView {
    NativeFeedAdView *currentAdView = _customAdView;
    if (currentAdView) {
        [currentAdView removeFromSuperview];
    }

    for (UIView *subview in [_containerView.subviews copy]) {
...
#pragma mark - LingAggNativeAdDelegate
