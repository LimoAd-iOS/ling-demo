...
@property (nonatomic, strong) LingAggNativeObj *nativeObj;
@property (nonatomic, strong) LingAggRewardVideoAd *rewardVideoAd;
@property (nonatomic, strong) LingAggSplashAd *hotSplashAd;
...
@property (nonatomic, strong) UIButton *tokenButton;
...
@property (nonatomic, strong) AdbidNativeTokenTester *tokenTester;
...
- (void)viewDidLoad {
    ...
    self.tokenTester = [[AdbidNativeTokenTester alloc] init];
...
- (void)setupUI {
    ...
    [self.controlPanel addSubview:self.tokenButton];
...
- (void)setupConstraints {
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.controlPanel.heightAnchor constraintEqualToConstant:450]
    ]];
    ...
    self.tokenButton.translatesAutoresizingMaskIntoConstraints = NO;
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        // C2S Token 加载按钮
        [self.tokenButton.topAnchor constraintEqualToAnchor:self.winNoticeButton.bottomAnchor constant:12],
        [self.tokenButton.leadingAnchor constraintEqualToAnchor:self.controlPanel.leadingAnchor constant:20],
        [self.tokenButton.trailingAnchor constraintEqualToAnchor:self.controlPanel.trailingAnchor constant:-20],
        [self.tokenButton.heightAnchor constraintEqualToConstant:44],

        // 状态标签
        [self.statusLabel.topAnchor constraintEqualToAnchor:self.tokenButton.bottomAnchor constant:12],
...
- (void)loadButtonTapped:(UIButton *)sender {
    ...
    LingAggNativeAd *nativeAd = [[LingAggNativeAd alloc] initWithSlotId:slotId];
...
- (void)loadC2SButtonTapped:(UIButton *)sender {
    ...
    nativeAd.rootViewController = self;
    nativeAd.delegate = self;
    nativeAd.shouldMuted = NO;
    self.nativeAd = nativeAd;

    NSLog(@"开始加载信息流C2S广告，广告位ID: %@", slotId);
    [self requestServerBidTokenConfigBeforeLoadForNativeAd:nativeAd slotId:slotId];
}

- (void)requestServerBidTokenConfigBeforeLoadForNativeAd:(LingAggNativeAd *)nativeAd
                                                  slotId:(NSString *)slotId {
    __weak typeof(self) weakSelf = self;
    [LingAggSDKManager requestServerBidTokenConfigForPositionId:slotId completion:^(NSString * _Nullable sdkInfoConfig, NSError * _Nullable configError) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf || strongSelf.nativeAd != nativeAd) {
            return;
        }

        if (configError) {
            NSLog(@"SDK Info获取失败: %@", configError.localizedDescription ?: @"unknown");
            [strongSelf updateStatus:AdStatusError];
            return;
        }
        ...
        [strongSelf.tokenTester getTokenWithAdId:slotId sdkInfo:sdkInfoConfig completion:^(BOOL success, NSDictionary * _Nullable config, NSError * _Nullable error) {
            if (strongSelf.nativeAd != nativeAd) {
                NSLog(@"忽略已失效的信息流C2S Token请求结果");
                return;
            }

            NSString *token = [config objectForKey:@"token"];
            if (!success || error || ![token isKindOfClass:[NSString class]] || token.length == 0) {
                NSLog(@"C2S Token获取失败: %@", error.localizedDescription ?: @"返回token为空");
                [strongSelf updateStatus:AdStatusError];
                return;
            }

            [nativeAd loadAdWithToken:token];
        }];
    }];
}
...
- (void)showFullscreenNativeAdInCurrentPage {
    ...
    // 必须挂在 self.view 上，不能挂 navigationController.view：
    // 广告 SDK 的落地页是 push 进导航栈的，被 push 页面的 view 位于 nav 内部的
    // UINavigationTransitionView 里；而挂在 nav.view 上的 overlay 是它的兄弟且 z 序更高，
    // 会把落地页整个盖住（表现为“点击弹出的 H5 看不见，关掉广告才露出来”）。
    // 挂到 self.view 后，push 进来的页面会覆盖当前 VC 的 view 及其所有子视图，层级才正确。
    // 导航栏默认 translucent，self.view 本身就是全屏尺寸，全屏效果不受影响。
    ...
    [self.fullscreenEpisodeScrollView setContentOffset:CGPointZero animated:NO];
    [self updateFullscreenEpisodePlayerLayerFrames];
...
- (void)setupFullscreenMuteButtonIfNeeded {
    ...
    self.fullscreenMuted = self.nativeAd.shouldMuted;
    UIView *containerView = self.fullscreenAdPageView ?: self.fullscreenAdOverlayView;
...
- (void)bindFullscreenAdContentIfNeeded {
    ...
    [self applyFullscreenMutedState];
    [self updateFullscreenEpisodePlaybackForCurrentPage];
    // 测试 SDK 几何可见性检查：广告视频仅在此起播一次，之后滑出/滑回不再由宿主控制，
    // 由 AdbidNativeMediaView 自身检测是否滑出可视区域来暂停/续播。
    if (self.nativeObj.isVideoAd) {
...
- (void)updateFullscreenEpisodePlaybackForCurrentPage {
    ...
    // 测试 SDK 几何可见性检查：此处不再随分页手动 pause/play 广告 mediaView。
    // 广告视频仅在绑定时(bindFullscreenAdContentIfNeeded)起播一次，
    // 之后滑出屏幕的暂停、滑回的续播完全交给 AdbidNativeMediaView 自身的可见性检测。
}
...
- (void)showImageNativeAd {
    ...
    if (self.nativeObj.imageAdInfo) {
        LingAggNativeImageObj * objc = self.nativeObj.imageAdInfo;
...
- (void)updateButtonStyles {
    ...
    self.tokenButton.alpha = self.tokenButton.enabled ? 1.0 : 0.6;
...
- (void)lossNoticeButtonTapped:(UIButton *)sender {
    ...
    if (self.nativeAd && self.nativeAd.data) {
        ...
        LingAggBidLossInfo *info = [[LingAggBidLossInfo alloc] init];
        info.winnerPrice = self.nativeAd.eCPM + 10; // 模拟竞胜价格高于我方
        info.winnerPlatform = LingAggPlatform_GDT; // 模拟广点通竞胜
...
- (void)setupHotSplashAd {
    ...
    self.hotSplashAd = [[LingAggSplashAd alloc] initWithSlotId:slotId];
...
- (void)splashAdDidFinishConversion:(LingAggSplashAd *)splashAd interactionType:(LingAggAdRedirectionType)interactionType {
    if (splashAd != self.hotSplashAd) {
        return;
    }
...
- (void)prefetchVideoCoverImageIfNeeded:(LingAggNativeObj *)nativeObj {
...
- (UIButton *)tokenButton {
    if (!_tokenButton) {
        _tokenButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_tokenButton setTitle:@"加载C2S广告" forState:UIControlStateNormal];
        [_tokenButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [self configureActionButton:_tokenButton backgroundColor:[UIColor colorWithRed:0.92 green:0.45 blue:0.12 alpha:1.0]];
        [_tokenButton addTarget:self action:@selector(loadC2SButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _tokenButton;
}
