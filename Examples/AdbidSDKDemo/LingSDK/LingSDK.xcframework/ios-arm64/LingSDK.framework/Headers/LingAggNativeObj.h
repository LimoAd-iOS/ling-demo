...
@class LingAggNativeVideoObj;
@class LingAggNativeImageObj;
@interface LingAggNativeObj : NSObject
...
/**
 视频是否由 adapter 接管渲染（YES 时 SDK 不在 LingAggNativeMediaView 上构造自己的播放器，
 adapter 会通过 extraConfig[@"mediaView"] 拿到 LingAggNativeMediaView 并把自家 SDK 的播放视图嵌进去）
 */
...
/**
 广告是图片，该属性有值
 */
@property (nonatomic, copy, readonly) LingAggNativeImageObj * imageAdInfo;

/**
 广告是视频，该属性有值
 */
@property (nonatomic, copy, readonly) LingAggNativeVideoObj * videoAdInfo;

@end

@interface LingAggNativeVideoObj : NSObject
...
@interface LingAggNativeImageObj : NSObject
