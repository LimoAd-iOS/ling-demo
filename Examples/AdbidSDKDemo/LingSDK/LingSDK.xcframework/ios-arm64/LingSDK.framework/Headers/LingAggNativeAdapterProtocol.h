...
/// 开屏广告适配层代理（回调给LingAggNativeAd）
@protocol LingAggNativeAdapterDelegate <NSObject>
@optional
/// 素材加载成功
/// data 素材
/// resource 本地素材
...
/// 开屏广告适配层抽象协议
@protocol LingAggNativeAdapterProtocol <NSObject>
/**
 必传.
 处理广告点击事件的根视图控制器。
 */
@property (nonatomic, weak, readwrite) UIViewController *rootViewController;
/// 适配层代理（回调至LingAggNativeAd）
@property (nonatomic, weak) id<LingAggNativeAdapterDelegate> adapterDelegate;
/// 广告最大加载时长（毫秒）
 ...
 *   - templateView   UIView*              模版广告的渲染容器，adapter/Taku 会将真实模版视图插入其中
 *   - mediaView      LingAggNativeMediaView*  App 持有的媒体视图,adapter 在视频广告时可把自家 SDK 的播放视图嵌进它
 *   - adFrame / mediaViewFrame / sizeToFit / adLogoFrame / adOptionsFrame / networkLogoFrame / videoPlayType
 *     模版广告专用,见 LingAggNativeAd.h 注释
 */
