...
@protocol LingBannerAdDelegate <NSObject>

/// 广告加载成功
- (void)bannerAdDidLoad:(LingBannerAd *)bannerAd;

/// 广告加载失败
- (void)bannerAd:(LingBannerAd *)bannerAd didFailToLoadWithError:(NSError *)error;

@optional

/// 广告被点击
- (void)bannerAdViewDidClick:(LingBannerAd *)bannerAd;

/// 广告曝光
- (void)bannerAdViewDidExpose:(LingBannerAd *)bannerAd;

/// 广告关闭按钮被点击（开发者需自行从父视图移除）
- (void)bannerAdViewWillClose:(LingBannerAd *)bannerAd;

/// 自动刷新即将开始（仅当 autoRefreshInterval > 0 时触发）
- (void)bannerAdViewWillRefresh:(LingBannerAd *)bannerAd;

@end

@interface LingBannerAd : UIView

@property (nonatomic, weak) id<LingBannerAdDelegate> delegate;
